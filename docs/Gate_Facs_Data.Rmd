...
## Constructing gates
=======
## GATING USING CYTOEXPLORER

## Create a gating set
This seems to work below.
```{r}
gs <- cyto_setup(path="/Users/david/Projects/data/flowdata/Accuri",
                 select="fcs")


##########
#gate_flow_data <- GatingSet(flowData)
gate_flow_data <- cyto_transform(gs)
cyto_gate_draw(gate_flow_data,
               gatingTemplate = "Manual-Gating1.csv",
               parent = "root",
               alias = "Cells",
...
## Constructing a gate
>>>>>>> cd44ac58a164b66850c350f5cb81a5bbec22b4d5

To perform gating, we use the [openCyto](http://opencyto.org/) package

### Gating debris

The first step is to gate out debris from our sample.  This is performed by plotting forward scatter versus side scatter 

```{r}
#gating_set <- GatingSet(flowData)
#gating_set
quartz()

plot(flowData[[1]], c('FSC.A','SSC.A'), xlim=c(0,3e6), ylim=c(0,1e6),smooth=T)
#autoplot(flowData[[1]], c('FSC.A','SSC.A'), xlim=c(0,3e6), ylim=c(0,1e6),smooth=T)
...
ggcyto(flowData.singlets[c(neg.signal,pos.signal,hi.signal)], aes(x = `FSC.A`, y =  `SSC.A`)) + geom_hex(bins = 512) + ggcyt
