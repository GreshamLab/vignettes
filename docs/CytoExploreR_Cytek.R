...
cytek_gating_set <- cyto_setup(path="/Users/david/Projects/data/flowdata/Cytek", select="fcs")

#To interactively edit the experiment details
#cyto_details_edit(cytek_gating_set)

#To tqke a look at the data
cyto_plot_explore(cytek_gating_set,
                  channels_x = "FSC-A",
                  channels_y = c("B2-A","V2-A")
)

cyto_plot_profile(cytek_gating_set,
                  parent = "root",
                 channels = c("FSC-A","B2-A")
                  )

cyto_plot_gating_tree(cytek_gating_set)
...
cytek_transformed <- cyto_transformer_logicle(cytek_gating_set)
cytek_FSC_SSC_transformed <- cyto_transformer_logicle(cytek_gating_set,
                                              channels = c("SSC-H", "SSC-A", "FSC-H", "FSC-A", "SSC-B-H", "SSC-B-A")
                                              )

cytek_combined_transformed <- cyto_transformer_combine(cytek_transformed,
                                  cytek_FSC_SSC_transformed)

transformed_cytek <- cyto_transform(cytek_gating_set,
                                    trans = cytek_combined_transformed)


#To tqke a look at the transformed data
cyto_plot_explore(transformed_cytek,
                  density_modal = TRUE,
                  axes_limits = "data",
              #    display = 100000,
                  point_col_alpha = 0.5,
...
cyto_plot_profile(transformed_cytek,
                  parent = "root",
                  channels = c("FSC-A","GFP","mCherry-A"), #add as many channels as needed.
                  legend = "fill"
)

cyto_plot_profile(transformed_cytek[1],
                  parent = "root"
)

View(cyto_stats_compute(transformed_cytek[2],
                   alias = "root",
                   stat = "median"
                   )
)
...
cyto_gate_draw(transformed_cytek,  #entire gating set is plotted for gating purposes.  It is downsamples to 25,000 events for
               parent = "root",
               alias = "Cells",
...
cyto_gate_draw(transformed_cytek,
               parent = "Cells",
               alias = "Single_cells",
...
cyto_plot_gating_scheme(transformed_cytek[6])

#To visualize the gating tree
cyto_plot_gating_tree(transformed_cytek)

#To visualize the freq of cells in each gate for a single sample
cyto_plot_gating_tree(transformed_cytek[[1]], stat="freq")

stats_freq <- cyto_stats_compute(transformed_cytek,
                             parent = "Single_cells",
...
stats_count1 <- cyto_stats_compute(transformed_cytek,
                                 parent = "Single_cells",
                                 alias = c("Negative", "One_copy", "Two_copy", "multi_copy"),
                                 stat="count",
...
cyto_gate_remove(transformed_cytek,
                 gatingTemplate = "cytek_gating.csv",
...
#cyto_plot_gating_scheme(transformed_cytek2[1:2], stat="freq")
...
negative <- cyto_extract(transformed_cytek, "Single_cells")[[1]]
...
#cyto_plot_gating_scheme(transformed_cytek[2], stat="freq")

##To overlay the one GFP copy cells when gating the two copy GFP we need to extract the relevant data
one_copy <- cyto_extract(transformed_cytek, "Single_cells")[[2]]
...
cyto_plot_gating_scheme(transformed_cytek[3], stat="freq")

##To overlay the two GFP copy cells when gating the more than two copies we need to extract the data
two_copy <- cyto_extract(transformed_cytek, "Single_cells")[[3]]
...
#cyto_plot_gating_scheme(transformed_cytek2, stat="freq")

cyto_plot_gating_tree(transformed_cytek[[7]],
                      stat="freq")

cyto_plot_gating_scheme(transformed_cytek[7],
                        back_gate = TRUE,
                        gate_track = TRUE)

stats_freq2 <- cyto_stats_compute(transformed_cytek,
                                  parent = "Single_cells",
                                  alias = c("Negative", "One_copy", "Two_copy", "multi_copy"),
                                  stat="freq",
                                  save_as = "stats_freq2.csv")

stats_count2 <- cyto_stats_compute(transformed_cytek,
                                   parent = "Single_cells",
                                   alias = c("Negative", "One_copy", "Two_copy", "multi_copy"),
                                   stat="count",
...
negative <- cyto_extract(transformed_cytek, "Single_cells")[[1]] #DGY1
##To overlay the one GFP copy cells  we need to extract the relevant data
one_copy <- cyto_extract(transformed_cytek, "Single_cells")[[2]] #DGY500
##To overlay the two GFP copy cells  we need to extract the data
two_copy <- cyto_extract(transformed_cytek, "Single_cells")[[3]] #DGY1315
...
cyto_gate_draw(transformed_cytek,
               ...
               gatingTemplate = "cytek_gating_three.csv",
...
stats3 <- cyto_stats_compute(transformed_cytek,
                             parent = "Single_cells",
                             alias = c("Negative", "One_copy", "Two_copy", "multi_copy"),
                             stat="freq")

stats_freq3 <- cyto_stats_compute(transformed_cytek,
                                  parent = "Single_cells",
                                  alias = c("Negative", "One_copy", "Two_copy", "multi_copy"),
                                  stat="freq",
                                  save_as = "stats_freq3.csv")

stats_count3 <- cyto_stats_compute(transformed_cytek,
                                   parent = "Single_cells",
                                   alias = c("Negative", "One_copy", "Two_copy", "multi_copy"),
                                   stat="count",
...
cyto_gate_edit(transformed_cytek,
               alias = "Cells",
               channels = c("FSC-A","SSC-A"),
               gatingTemplate = "cytek_test_data.csv",
               type = "boundary")
...
cyto_plot_gating_scheme(transformed_cytek[[3]],
                        back_gate = TRUE,
                        gate_track = TRUE)

cyto_stats_compute(transformed_cytek,
...
cyto_plot_explore(transformed_cytek[1],
                  channels_x = "FSC-A",
                  channels_y = "GFP"
)


cyto_plot(transformed_cytek,
          parent="Cells",
          channels = "FSC-A",
     #    alias = "",
...
cyto_plot(transformed_cytek[1:19],
          parent="Cells",
          channels = "GFP",
        #  label_text = "Strain",
        legend = "line",
